...
class CadastroClienteForm(FlaskForm):
    ...
    email = StringField(
        "E-mail",
        validators=[
            DataRequired(message="O e-mail e obrigatorio."),
            Length(min=5, max=120, message="O e-mail deve ter entre 5 e 120 caracteres."),
            Regexp(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", message="Formato de e-mail invalido.")
        ]
    )
    ...
    def validate_senha(self, field):
        ...
        if field.data in ("1234", "12345678", "password", "senha123", "admin123", "itclube", "itclube2026"):
            raise ValidationError("Esta senha e muito comum e insegura. Escolha outra senha.")

    def validate_email(self, field):
        """Verifica duplicidade de e-mail no banco de dados."""
        email_limpo = (field.data or "").strip().lower()
        if not email_limpo:
            return
        cliente_existente = Cliente.query.filter_by(email=email_limpo).first()
        if cliente_existente:
            raise ValidationError("Este e-mail ja esta cadastrado no sistema.")
