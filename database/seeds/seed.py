"""Popula o banco com funcionarios em diferentes cargos, clientes, compras, ledger e recompensas para o IT Clube."""
import os
import sys

# Insere backend no sys.path
...
with app.app_context():
    ...
    vendedor.set_senha("ITClube2026")

    db.session.add_all([admin, gerente, vendedor])
    db.session.flush()

    print("Proprietário criado: admin@loja.com (senha: ITClube2026)")
    print("Gerente criada: gerente@loja.com (senha: ITClube2026)")
    print("Vendedor criado: vendedora@loja.com (senha: ITClube2026)")

    # 2. Recompensas do IT Clube
    ...
    recompensa_premium = Recompensa(
        id_usuario=admin.id_usuario,
        nome="Kit Premium IT Clube",
        custo_pontos=25,
        tipo="produto_fisico",
        valor_beneficio=None,
        validade=(agora + timedelta(days=120)).date(),
        quantidade_total=5,
        quantidade_disponivel=5,
        status="ativa",
    )
    db.session.add_all([recompensa_brinde, recompensa_desconto, recompensa_premium])
    db.session.flush()

    # 3. Clientes Exemplo com Senha, Histórico e Ledger (Regra: 1 compra = 1 ponto)
    exemplos = [
        ("Ana Silva", "11999991111", "ana@ex.com", 12),
        ("Carlos Oliveira", "11988882222", "carlos@ex.com", 8),
    ]

    for nome, tel, email, pts in exemplos:
        cli = Cliente(nome=nome, telefone=tel, email=email, pontos_acumulados=pts, ativo=True)
        cli.set_senha("ITClube2026")
        db.session.add(cli)
        db.session.flush()

        print(f"Cliente {nome} ({tel}) criado com pontos e extrato (senha: ITClube2026)")
        ...
        if tel == "11999991111":
            ...
            m1 = MovimentacaoPontos(
                ...
                saldo_posterior=16,
                origem="vendedora",
                motivo="Compra registrada (+1 ponto)",
        ...
        if tel == "11988882222":
            ...
            m4 = MovimentacaoPontos(
                ...
                saldo_posterior=8,
                origem="iot_device",
                motivo="Compra via terminal ESP8266 (+1 ponto)",
    ...
    db.session.add_all([
        Auditoria(
            ...
            detalhes='{"nome": "Brinde IT Clube", "custo_pontos": 5}',
        ...
        Auditoria(
            ...
            detalhes='{"nome": "15% de Desconto Especial", "custo_pontos": 10}',
            data_hora=agora - timedelta(days=6),
        ),
        Auditoria(
            id_usuario=admin.id_usuario,
            ...
            entidade="resgate",
            entidade_id="1",
            detalhes='{"id_cliente": 1, "pontos": 5}',
            data_hora=agora - timedelta(days=1),
        ),
    ])

    db.session.commit()
    print("Seed do IT Clube concluído com sucesso!")
